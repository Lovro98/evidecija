...
async function fetchAll(isAdmin) {
  ...
  return {
    ...
    objects: (objects.data || []).map((o) => ({ id: o.id, name: o.name, billRate: Number(billMap[o.id]) || 0, country: o.country || "HR" })),
...
function TabBar({ tab, setTab }) {
  const tabs = [["radnici","Radnici","👷"],["objekti","Objekti","🏨"],["imenik","Imenik","📇"],["sati","Sati","🕒"],["isplate","Isplate","💶"],["obracun","Obračun","🧾"]];
  return (
...
function ObjectSelect({ data, api, value, onChange }) {
  ...
  return (
    <select value={value} onChange={(e) => (e.target.value === "__new__" ? setAdding(true) : onChange(e.target.value))}>
      ...
      {["HR", "CZ"].map((c) => {
        const objs = data.objects.filter((o) => (o.country || "HR") === c);
        return objs.length ? (
...
function ObjectsTab({ data, api, onOpenObject }) {
  ...
  const addObject = async () => {
    const n = newObj.trim(); if (!n) return;
    if (data.objects.find((o) => o.name.toLowerCase() === n.toLowerCase())) { setNewObj(""); return; }
    await api.addObject(n, newCountry); setNewObj("");
  };
  const list = [...data.objects].sort((a, b) => a.name.localeCompare(b.name, "hr"));
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              ...
              {api.admin && o.billRate > 0 && (
                <div className="num" style={{ textAlign: "right" }}>
                  <div style={{ fontWeight: 700, color: S.amber }}>{eur(revenue)}</div>
  ...
  return (
    <>
      <div style={{ fontSize: 17, fontWeight: 700, marginBottom: 10 }}>🏨 Objekti ({list.length})</div>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 14 }}>
        {["HR", "CZ"].map((c) => (
          <button key={c} onClick={() => setNewCountry(c)} style={{ padding: "7px 14px", borderRadius: 999, fontWeight: 700, fontSize: 13, cursor: "pointer",
            background: newCountry === c ? S.blue : "#fff", color: newCountry === c ? "#fff" : S.sub, border: `1px solid ${newCountry === c ? S.blue : S.line}` }}>
            {FLAG[c]} {COUNTRY_NAME[c]}
          </button>
        ))}
      </div>
      {list.length === 0 ? <Empty text="Još nema objekata. Dodaj prvi gore." /> : ["HR", "CZ"].map((c) => {
        const objs = list.filter((o) => (o.country || "HR") === c);
        if (!objs.length) return null;
        return (
          <div key={c}>
            <div style={{ fontSize: 14, fontWeight: 800, color: S.blue, padding: "8px 4px 6px" }}>{FLAG[c]} {COUNTRY_NAME[c]} ({objs.length})</div>
    ...
    <>
        ...
        return (
          <Card style={{ background: S.amberSoft, borderColor: "#EBD9B4" }}>
            ...
            <div className="num" style={{ fontSize: 14 }}>
              ...
              {extraCost > 0 && <div style={{ display: "flex", justifyContent: "space-between", padding: "3px 0" }}><span>Troškovi objekta (gorivo, avansi…)</span><b>−{eur(extraCost)}</b></div>}
              {extraCostKc > 0 && <div style={{ display: "flex", justifyContent: "space-between", padding: "3px 0" }}><span>Troškovi objekta u krunama</span><b>−{czk(extraCostKc)}</b></div>}
              <div style={{ display: "flex", justifyContent: "space-between", padding: "5px 0 0", borderTop: `1px dashed #EBD9B4`, fontWeight: 800 }}>
                <span>Dobit ovaj mjesec</span><span style={{ color: revenue - cost - extraCost >= 0 ? S.green : S.red }}>{eur(round2(revenue - cost - extraCost))}</span>
              </div>
            </div>
      ...
      {(() => {
        ...
        const rows2 = [...wStats.entries()]
          ...
          .filter((r) => r.w)
          .sort((a, b) => b.h - a.h || a.w.name.localeCompare(b.w.name, "hr"));
...
function PaymentsTab({ data, api }) {
  ...
  return (
    <>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
        {[["radnik", "👷 Za radnika"], ["objekt", "🏨 Za objekt"]].map(([id, label]) => (
      ...
      {target === "radnik" && data.workers.filter((x) => !x.archived).length === 0 ? <Empty text="Prvo dodaj radnika." /> : (
        <Card>
          {target === "radnik" ? (
            ...
            <>
              <Field label="Objekt">
                <ObjectSelect data={data} api={api} value={form.objectId} onChange={(v) => setForm({ ...form, objectId: v })} />
              </Field>
              <Field label="Vrsta">
          ...
          <div style={{ display: "flex", gap: 10 }}>
            ...
            <div style={{ flex: 1 }}>
              <Field label={cur === "CZK" ? "Iznos (Kč)" : "Iznos (€)"}>
...
function calcRows(data, filterFn) {
  return data.workers.map((w) => {
    ...
    return { w, logs, pays, hours, gross, rateSet,
      ...
      czk: { ...k, net: round2(k.bonuses - k.advances - k.deductions) } };
...
function ReportTab({ data, api, admin }) {
  ...
  }), { hours: 0, gross: 0, net: 0, firm: 0, bonus: 0, netKc: 0, firmKc: 0 });
  ...
  periodLogs.forEach((l) => {
    ...
    const cur = byObject.get(key) || { hours: 0, gross: 0, revenue: 0, costs: 0 };
  ...
  objPays.forEach((p) => {
    const cur = byObject.get(p.objectId) || { hours: 0, gross: 0, revenue: 0, costs: 0, costsKc: 0 };
  ...
  for (let i = 5; i >= 0; i--) {
    ...
    if (admin) {
      ...
      const extra = data.payments.filter((p) => monthKey(p.date) === mk2 && (p.currency || "EUR") === "EUR" && (!p.workerId ? !!p.objectId : (p.type === "bonus" || (!p.deduct && p.type !== "avans")))).reduce((s, p) => s + p.amount, 0);
  ...
  const printPayslip = (r) => {
    ...
    printDoc(`Obračun ${r.w.name}`, `
      ...
      ${logsHtml}${paysHtml}
      <tr class="tot"><td colspan="3">UKUPNO ${fmtH(r.hours)}</td><td></td><td class="right">${eur(r.net)}${r.czk.net !== 0 ? "<br>" + (r.czk.net > 0 ? "+" : "−") + czk(Math.abs(r.czk.net)) : ""}</td></tr></table>
      <div class="muted">Zarada ${eur(r.gross)}${r.bonuses ? " + bonus " + eur(r.bonuses) : ""}${r.advances ? " − avans " + eur(r.advances) : ""}${r.deductions ? " − odbici " + eur(r.deductions) : ""} = <b>za isplatu ${eur(r.net)}</b>${r.czk.net !== 0 ? " uz " + (r.czk.net > 0 ? "+" : "−") + czk(Math.abs(r.czk.net)) + " u krunama" : ""}</div>
  ...
  const exportExcel = () => {
    ...
    const objRows = [...byObject.entries()].map(([k, v]) => ({
      ...
      ...(admin ? { "Naplata (€)": v.revenue, "Dobit (€)": round2(v.revenue - v.gross - (v.costs || 0)) } : {}),
    }));
        ...
        <>
          ...
          <Card style={{ background: S.ink, color: "#fff", border: "none" }}>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, fontSize: 13.5 }}>
              ...
              <div><div style={{ color: "#9DB3A8" }}>Za isplatiti</div>
              ...
              <div><div style={{ color: "#9DB3A8" }}>Troškovi firme</div>
          ...
          {admin && (
            <Card style={{ background: S.amberSoft, borderColor: "#EBD9B4" }}>
              ...
              <div className="num" style={{ fontSize: 14 }}>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", padding: "3px 0" }}><span>Plaće + bonusi + svi troškovi</span><b>−{eur(totalCost)}</b></div>
            ...
            <Card style={{ background: S.blueSoft, borderColor: "#CBDCEA" }}>
              ...
              {["HR", "CZ", "__none__"].map((cGroup) => {
                ...
                const sub = entries.reduce((t, [, v]) => ({
                  hours: round2(t.hours + v.hours), gross: round2(t.gross + v.gross),
                  revenue: round2(t.revenue + v.revenue), costs: round2(t.costs + (v.costs || 0)), costsKc: round2(t.costsKc + (v.costsKc || 0)),
                }), { hours: 0, gross: 0, revenue: 0, costs: 0, costsKc: 0 });
                return (
                  <div key={cGroup}>
                    {cGroup !== "__none__" && (
                      <div className="num" style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 0 3px", fontWeight: 800, fontSize: 13.5, color: S.blue }}>
                        ...
                        <span>{fmtH(sub.hours)}{admin ? <> · dobit {eur(round2(sub.revenue - sub.gross - sub.costs))}{sub.costsKc > 0 ? <span style={{ color: S.red }}> −{czk(sub.costsKc)}</span> : null}</> : null}</span>
                      </div>
                    )}
                    {entries.map(([key, v]) => {
                ...
                return (
                  <div key={key} style={{ padding: "7px 0", borderBottom: `1px solid #CBDCEA` }}>
                    <div className="num" style={{ display: "flex", justifyContent: "space-between", fontSize: 14 }}>
                      ...
                      <span>{fmtH(v.hours)} · <b>{eur(v.gross)}</b>{v.costs > 0 ? <span style={{ color: S.red }}> · troškovi {eur(v.costs)}</span> : null}{v.costsKc > 0 ? <span style={{ color: S.red }}> · {czk(v.costsKc)}</span> : null}</span>
                    </div>
                    {admin && key !== "__none__" && (
                      <>
                        <div className="num" style={{ display: "flex", justifyContent: "space-between", fontSize: 12.5, color: S.amber, fontWeight: 700 }}>
                          ...
                          <span style={{ color: v.revenue - v.gross - (v.costs || 0) >= 0 ? S.green : S.red }}>dobit {eur(round2(v.revenue - v.gross - (v.costs || 0)))}</span>
                        </div>
                        {view === "month" && (
