...
async function fetchAll(isAdmin) {
  ...
  return {
    ...
    objects: (objects.data || []).map((o) => ({ id: o.id, name: o.name, billRate: billMap[o.id]?.rate || 0, billCur: billMap[o.id]?.cur || "EUR", country: o.country || "HR" })),
...
function ReportTab({ data, api, admin }) {
  ...
  const printEnvelopes = () => {
    ...
    printDoc("Koverte " + periodLabel, `<style>
      @page{size:220mm 110mm;margin:0}
      body{padding:0!important;margin:0!important;max-width:none!important}
      .env{width:220mm;height:110mm;box-sizing:border-box;padding:10mm 18mm;page-break-after:always;
        text-align:center;display:flex;flex-direction:column;justify-content:space-between}
      .env-top{font-size:13pt;color:#444;line-height:1.4}
      .env-name{font-size:22pt;font-weight:bold;letter-spacing:.5px;margin:2mm 0 1mm;text-transform:uppercase}
      .env-hours{font-size:13pt}
      .env-total{border-top:2px solid #000;padding-top:4mm;font-size:20pt;font-weight:bold;line-height:1.3}
    </style>${pages}`);
  };
  ...
  return (
    <>
      ...
      {rows.length === 0 ? <Empty text="Za ovo razdoblje nema upisanih sati ni isplata." /> : (
        <>
          <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
            <Btn kind="excel" onClick={exportExcel} style={{ flex: 1 }}>📊 Excel</Btn>
            <Btn kind="ghost" onClick={printEnvelopes} style={{ flex: 1, fontWeight: 700 }}>🖨 Koverte za isplatu</Btn>
          </div>
          ...
          <Card style={{ background: S.ink, color: "#fff", border: "none" }}>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, fontSize: 13.5 }}>
              ...
              <div><div style={{ color: "#9DB3A8" }}>Zarada + bonusi</div>
